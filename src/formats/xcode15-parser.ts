 ...
 * Uses the `xcresulttool get object` command (without --legacy flag for latest format)
 */
export class Xcode15Parser implements FormatParser {
  name = 'xcode15';
...
/**
 * Get test results using Xcode 15.x command format
 */
export async function getXcode15TestResults(bundlePath: string): Promise<any> {
  try {
    const { stdout } = await execa('xcrun', [
      'xcresulttool',
      'get',
